                        ...
                        @else
                            ...
                            <select name="kategori_id" class="form-control @error('kategori_id') is-invalid @enderror"
                                ...
                                @foreach ($kategori as $k)
                                    <option value="{{ $k->id }}"
                                        {{ isset($produk) ? ($k->id == $produk->kategori_id ? 'selected' : '') : '' }}>
                                        {{ $k->name }}</option>
                                @endforeach
                            </select>
                            @error('kategori_id')
                            ...
                            <input type="file" name="gambar"
                                class="form-control @error('gambar') is-invalid @enderror" placeholder="Gambar"
                            ...
                            @isset($produk)
                                <img src="/{{ $produk->gambar }}" width="300px" alt="">
                            @endisset

                            <br>
                            <a href="/admin/produk" class="btn btn-secondary mt-2"><i class="fas fa-arrow-left"></i>
                                Kembali</a>
                            <button type="submit" class="btn btn-primary mt-2"><i class="fas fa-save"></i>
                                Simpan</button>
                        </form>
                </div>
            </div>
